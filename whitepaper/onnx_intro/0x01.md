...
算子在 ATen 中已经实现了，ONNX 中也有相关算子的定义，但是相关算子映射成 ONNX 的规则没有写。在这种情况下，我们只需要**为 ATen 算子补充描述映射规则的符号函数**就行了。
...
> * `.pyi`文件是一个Python skeleton，具有适当的结构、调用签名和返回值，以匹配模块中定义的函数、属性、类和方法。
