...
动态范围量化将所有权重值从浮点精度转换为 8 位整数精度。为了进一步减少推理延迟，<mark style="color:red;">动态范围根据激活值的范围将其动态量化为 8 位整数</mark>，并使用 8 位整数的权重和激活值进行计算。乘法和累加运算完成之后，激活值将被去量化。下图显示了动态范围量化的整体机制。这种优化使推理延迟接近完全定点推理。
